...
void main(){
    ...
    float d = length(cv);
    float mirrors = 6;
    vec2 da = vec2(pow(d*10., 0.5), cos(mirrors*atan(cv.y, cv.x)));
    float t = time/4;
    float tr = 0.5;
    vec2 timeWheel = 10+vec2(cos(t), sin(t))*tr;
    vec2 daCopy = da.xy;
    da.x += noise(daCopy, timeWheel, .5, 0.3);
    da.y += noise(daCopy, timeWheel, .5, 0.3);
    float pct = abs(
        noise(da, timeWheel, 1., 1.0) +
        noise(da, timeWheel, 0.5, 2.0) +
         noise(da, timeWheel, 0.5, 10.0)
    );
