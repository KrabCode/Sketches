...
void main() {
  ...
  float scale = 300;
  float x = map(position.x+100., -scale, scale, -1, 1);
  float y = map(position.y-50., -scale, scale, -1, 1);
  ...
  float tempX = x;
  x = x+0.5*fbm(vec4(x,y,t.x, t.y));
  y = y+0.5*fbm(vec4(y,tempX,t.x, t.y));

  vec2 myPos = vec2(x,y)*1.;

  float n = 1.-smoothstep(.0, 0.35, abs(fbm(vec4(myPos.x, myPos.y, t.xy))));
  vec4 pos = position + vec4(0, n*400., 0, 0);
