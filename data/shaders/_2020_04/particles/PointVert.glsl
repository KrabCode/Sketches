...
void main() {
  float rand = random(position.x);
  float r = 50+150.*rand;
  float t = time*.5;
  float x = r*cos(t+rand*130);
  float y = r*cos(t+rand*260);
  float z = r*sin(t+rand*10);


  vec4 pos = modelviewMatrix * (position*vec4(0,1,1,1)+vec4(x,y,z,1));
