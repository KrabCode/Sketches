...
void main() {
    ...
    float d = map(length(position), minRadius, maxRadius, 0, 1);
    float y = map(abs(position.y), 0, minRadius, 0, 1);
    vec4 clr = vec4(getColor(d).rgb, 1);
    clr = mix(vec4(1), clr, clamp(pow(1.6-y, 12.), 0, 1));
    vec4 myAmbient = clr;
    vec4 myColor = clr;
    vec4 mySpecular = clr;
    ...
    vertColor =     vec4(totalAmbient, 0) * myAmbient * specular +
    vec4(totalFrontDiffuse, 1) * myColor * specular  +
    vec4(totalFrontSpecular, 0) * specular +
    vec4(emissive.rgb, 0);

    backVertColor = vec4(totalAmbient, 0) * myAmbient +
    vec4(totalBackDiffuse, 1) * myColor +
