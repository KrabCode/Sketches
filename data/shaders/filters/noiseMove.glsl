...
void main(){
    ...
    float angle = dotNoise2D();
    vec2 noiseOffset = vec2(cos(angle), sin(angle));
    vec4 moved = texture(texture, uv+noiseOffset*moveDist);
    gl_FragColor = prev + moved*moveStrength;
}