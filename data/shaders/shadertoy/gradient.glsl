...
int findClosestLeftNeighbourIndex(float pos, int colorCount){
    for(int i = 0; i < 100; i++){
...
vec4 getColor(float n, int colorCount, vec4[100] colorValues, float[100] colorPositions){
...
void main(){
    ...
    vec4[] colorValues = vec4[](vec4(1,1,1,1), vec4(1,1,1,1), vec4(1,1,1,1));
    float[] colorPositions = float[](0,.5,1);
    gl_FragColor = getColor(uv.y, colorValues.length, colorValues, colorPositions);
}